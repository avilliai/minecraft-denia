# Minecraft Girlfriend (Fabric 模组)

一个由 LLM 驱动的 Minecraft「女友」伴随实体：会跟随、会聊天、会撒娇，回复同时通过
...
- **召唤伴随实体**：`/gf summon` 在身边生成一个人形女友，绑定你为主人。
- **跟随 AI**：跟随主人、闲逛、看向玩家、自动浮水。
- **聊天接 LLM**：在她附近（默认 16 格内）发普通聊天 → 异步调用 LLM → 回复显示在
...
| 命令 | 作用 |
|---|---|
| `/gf summon` | 在身边召唤女友并绑定为主人 |
| `/gf come` / `/gf follow` | 让她过来 / 跟随你 |
| `/gf stop` | 停在原地 |
| `/gf say <文本>` | 让她说一句（测试语音/气泡） |
| `/gf reload` | 重新加载配置文件 |
